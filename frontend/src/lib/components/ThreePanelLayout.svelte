<script lang="ts">
	...
	const guideSteps = [
		...
		{
			selector: '[data-tour="summary-tab"]',
			title: 'Summary Tab',
			description: 'After running a tool, the Summary tab shows key statistics and metrics from the output - like read counts, quality scores, and assembly stats.',
			position: 'bottom' as const
		},
		{
			selector: '[data-tour="output-tab"]',
			title: 'Output Tab',
			description: 'The Output tab lists all files generated by the current tool. Click View to inspect HTML reports, images, or text files.',
			position: 'bottom' as const
		},
		{
			selector: '[data-tour="notes-tab"]',
			title: 'Notes Tab',
			description: 'The Notes tab provides helpful context about file formats and what each output file contains.',
			position: 'bottom' as const
		},
		{
			selector: '[data-tour="output-files"]',
			title: 'Output Files',
			description: 'This dropdown shows ALL output files generated across every tool you\'ve run in the session. Use it to revisit earlier results.',
			position: 'bottom' as const
		}
	];
...
<div class="h-screen w-screen flex flex-col overflow-hidden" style="display: flex; flex-direction: column; height: 100%; width: 100%;">
	...
	<div id="main-content" class="flex-1 flex overflow-hidden" style="display: flex; flex: 1; overflow: hidden; height: calc(100% - 40px); min-height: 0;">
		...
		<div class="story-panel overflow-auto" style="width: {100 - leftPanelWidth}%; height: 100%; overflow: auto;">
			{#if guideMode && !storyline}
				<div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; padding: 2rem; text-align: center; background: linear-gradient(135deg, #eff6ff 0%, #f0fdf4 100%);">
					<div style="font-size: 3rem; margin-bottom: 1rem;">Welcome!</div>
					<h2 style="font-size: 1.5rem; font-weight: 700; color: #1f2937; margin-bottom: 0.75rem;">Biolearn Interface Guide</h2>
					<p style="color: #4b5563; max-width: 400px; line-height: 1.6; margin-bottom: 1.5rem;">
						This short tour will walk you through the main parts of the Biolearn interface. You'll learn where to type commands, view results, and find output files.
					</p>
					<p style="font-size: 0.875rem; color: #9ca3af;">The guided tour will start in a moment...</p>
				</div>
			{:else}
...
{#if showGuide}
	<SpotlightOverlay steps={guideSteps} onComplete={handleGuideComplete} />
{/if}
